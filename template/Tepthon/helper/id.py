...
async def _(event):
    ...
    if input_str := event.pattern_match.group(2):
        ...
        try:
            if p.first_name:
                return await edit_or_reply(
                    event, f"**⎉╎أيــدي المستخـدم**  `{input_str}` **هـو** `{p.id}`"
                )
        except Exception:
            try:
                if p.title:
                    return await edit_or_reply(
                        event, f"**⎉╎أيــدي المستخـدم**  `{p.title}` **هـو** `{p.id}`"
                    )
            except Exception as e:
                LOGS.info(str(e))
        ...
        if r_msg.media:
            ...
            await edit_or_reply(
                event,
                f"**⎉╎أيــدي الدردشـه : **`{event.chat_id}`\n\n**⎉╎أيــدي المستخـدم : **`{r_msg.sender_id}`\n\n**⎉╎أيــدي الميديـا : **`{bot_api_file_id}`",
            )
        ...
        await edit_or_reply(event, f"**⎉╎أيــدي الدردشـه : **`{event.chat_id}`")
...
@Tepthon_cmd(
    ...
    info={
        "header": "لـ جـلب اسـم الشخـص بشكـل ماركـدون ⦇.رابطه بالـرد او + معـرف/أيــدي الشخص⦈ ",
...
@Tepthon_cmd(
    ...
    info={
        "header": "لـ جـلب اسـم الشخـص بشكـل ماركـدون ⦇.اسمه بالـرد او + معـرف/أيــدي الشخص⦈ ",
