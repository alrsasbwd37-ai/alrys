...
❃ `{i}اعادة`
    لـ رجوع الحساب إلى وضعه الطبيعي
    
⚠️ تـنبيـه : هذا الأمر قد يكون مخالف لسياسة التليجرام إذا تم التبليغ عنك بصفة انتحال لذا استعمله حسب مسؤوليتك
"""
...
@Tepthon_cmd(pattern="انتحال ?(.*)", fullsudo=True)
async def _(event):
    eve = await event.eor("**⎆ جار انتحال المستخدم...**")
    ...
    await event.client.send_message(
        event.chat_id, f"⎆ تم بنجاح انتحال حساب المستخدم**\n❃ أنا `{first_name}` من الآن**", reply_to=reply_message
    )


@Tepthon_cmd(pattern="اعادة$")
async def _(event):
    name = OWNER_NAME
    ok = ""
    mybio = str(jmubot.me.id) + "01"
    ...
    await event.eor("**⎆ تم بنجاح إعادة الحساب إلى وضعه الطبيعي ✅**")
